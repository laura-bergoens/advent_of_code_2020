const fs = require('fs');
const { jobGetHigher, jobFindAvailable } = require('./job');
...
async function main() {
  try {
    ...
    const solution1 = jobGetHigher(rawInput);
    console.log(`solution higher seat: ${solution1}`);
    const solution2 = jobFindAvailable(rawInput);
    console.log(`solution available: ${solution2}`);
  } catch (error) {
    console.error('\n', error);
    process.exit(1);
  }
