const fs = require('fs');
const { jobOnlyOnce, jobReplace } = require('./job');
...
/**
 ...
 as an offset from the jmp instruction; for example, jmp +2 would skip the next instruction, jmp +1
 would continue to the instruction immediately below it, and jmp -20 would cause
...
async function main() {
  try {
    ...
    const solution1 = jobOnlyOnce(rawInput);
    console.log(`solution1: ${solution1}`);
    const solution2 = jobReplace(rawInput);
